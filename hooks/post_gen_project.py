...
def copy_files_to(path):
    ...
    for file in filenames:
        if os.path.isfile(file):
            print(INFO + "Copying {} to current directory...".format(file) + TERMINATOR)
            shutil.copy(file, path)

    for directory in directories:
        print(INFO + "Copying {} to current directory...".format(directory) + TERMINATOR)
        dest = os.path.join(path, os.path.basename(directory))
        os.mkdir(dest)
